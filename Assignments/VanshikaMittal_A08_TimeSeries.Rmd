...
```{r}
#15 
GaringerOzone.monthly.Components <- as.data.frame(ozone.monthly.decomp$time.series[,2:3])

nonseasonal.GaringerOzone.monthly <- c(GaringerOzone.monthly.Components$trend, 
                                             GaringerOzone.monthly.Components$remainder)
#assign a new ts object 
nonseasonal.GaringerOzone.monthly.ts <- ts(nonseasonal.GaringerOzone.monthly, start= c(f_year, f_month), frequency = 12)

#16
nonseasonal.GaringerOzone.monthly.ts.trend <- Kendall::MannKendall(nonseasonal.GaringerOzone.monthly.ts)
summary(nonseasonal.GaringerOzone.monthly.ts.trend)
```

> Answer: Our p-value is very close to 0 ( a value less than 0.5) which means we can reject the null hypothesis and concludde that the our data is not stationary and has a trend over the given period of time. These results are comparable to the Seasonal MannKendall test which I ran earlier as they both echo the same result. 
