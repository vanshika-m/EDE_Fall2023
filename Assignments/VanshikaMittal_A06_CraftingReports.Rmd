...
```{r echo=FALSE, fig.cap= "Ozone levels distributed across months in 2018", fig.height = 4, fig.width = 7, fig.align='left'}
Ozone_2018 <- epa_data %>%
  filter(Year == 2018)

plot_ozone_2018 <- ggplot(Ozone_2018) +
              geom_boxplot(aes(x= factor(Month, levels = 1:12, labels = month.abb), 
                         y = Ozone)) +
              labs(x="Month", y= " Ozone Concentration(mu g/m^3)")
plot_ozone_2018
```

```{r echo=FALSE, fig.cap = "Ozone levels distributed across months in 2019",fig.height = 4, fig.width = 7, fig.align='left'}
Ozone_2019 <- epa_data %>%
  filter(Year == 2019)

plot_ozone_2019 <- ggplot(Ozone_2019) +
              geom_boxplot(aes(x= factor(Month, levels = 1:12, labels = month.abb), 
                         y = Ozone)) +
              labs(x ="Month", y = " Ozone Concentration(mu g/m^3)")
...
## Git Repository
https://github.com/vanshika-m/EDE_Fall2023/blob/main/Assignments/VanshikaMittal_A06_CraftingReports.Rmd
