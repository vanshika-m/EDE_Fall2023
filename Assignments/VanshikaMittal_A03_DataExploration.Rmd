...
Litter <- read.csv("./Data/Raw/NEON_NIWO_Litter_massdata_2018-08_raw.csv", stringsAsFactors = TRUE, header = TRUE)
```
...
summary(Neonics$Species.Common.Name, decreasing = FALSE) #summarising and sorting in descending order to find the most commonly studied species 
```

> Answer: Honey Bee, Parasitic Wasp, Buff Tailed Bumblebee, Carniolan Honey Bee, Bumble Bee and Italian Honeybee
...
> Answer: There are certain cells in this column which contain the vlaue "NR" which could be the possibel reason behind R reading it as a string value. While loading the data into our script, we changed as string values to be read as factors
